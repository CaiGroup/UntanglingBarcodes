...
<i> results/mpaths_ch_*_pos_*_lf*_wf*_sf*_df*.csv </i> files list each decoded barcode in the experiment with many columns:
  ...
  * <b>gene_name</b> gives the name of the gene for which the barcode represents (negative control means that barcode is for codeword that was unassigned to a gene)
...
A list of all non-zero counted barcodes for each cell and combination of decoding parameters in the experiment is in <i> cell_barcode_counts.csv </i>.
To proceed with data analysis, you will need to first subset to a single combination of decoding parameters (lvf, wf, svf, dr) and remove negative control barcodes. 

After reading the <i> cell_barcode_counts.csv </i> into python as a pandas dataframe you can remove negative controls and find gene-by-cell matrices for each combination of decoding parameters using the pandas functions:

<pre> <code>
# remove negative controls
results = results.loc[results.gene_name != "negative_control"]

cell_gene_counts = results.set_index(["lvf", "wf", "svf", "dr", "pos", "cellid", "gene_name"])

#compute gene-by-cell matricies for each decoding parameter set
cell_gene_mats = cell_gene_counts.unstack("gene_name").fillna(0)
</pre> </code>

The resulting data frame, <i> cell_gene_mats </i>, will contain each gene-by-cell matrix indexed by the syndrome decoding parameters that generated them.