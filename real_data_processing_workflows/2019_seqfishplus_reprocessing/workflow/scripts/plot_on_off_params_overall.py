...
summary_stats = pd.read_csv(snakemake.input[0])

grpd = summary_stats.groupby(['lvf', 'lwvf', 'svf', 'dr'])

stats_summed = grpd.sum()

#print(stats_summed)
stats_summed.reset_index(inplace=True)
...
rw['Gene Encoding Barcodes (counts)'] = stats_summed['gene_encoding_barcodes']
rw['Negative Control Barcodes (counts)'] = stats_summed['negative_control_barcodes']
rw["Lateral Variance Penalty"] = stats_summed['lvf']
rw["Number of Allowed Drops"] = stats_summed['dr']
rw["Log(weight) Variance Penalty"] = stats_summed['lwvf']
...
sns.relplot(data=rw, x="Negative Control Barcodes (counts)", y = "Gene Encoding Barcodes (counts)", hue="Lateral Variance Penalty", size="Number of Allowed Drops", style = "Log(weight) Variance Penalty")
