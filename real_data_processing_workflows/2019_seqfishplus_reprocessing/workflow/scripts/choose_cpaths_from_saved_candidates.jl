...
println("finished optimizations")
...
n_codewords = nrow(cb)
println("n_ontargets = n_codewords - sum(cb.gene .== negative_control)")
n_ontargets = n_codewords - sum(cb.gene .== "negative_control")

n_barcodes = nrow(mpaths)
println("n_neg_cntrl_mpaths = sum(mpaths.gene .== negative_control)")
n_neg_cntrl_mpaths = sum(mpaths.gene .== "negative_control")
println("build sum stats df")
sum_stats = DataFrame()
sum_stats[!, "pos"] = [pos]
sum_stats[!, "cell"] = [cell]
sum_stats[!, "ch"] = [ch]
