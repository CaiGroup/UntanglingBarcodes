...
class CrossChannelFMMatcher:
    def __init__(self, ro, ref, ref_final=None, eps_xy=0, eps_z=0):
        #ref.reset_index(inplace=True)

        self.channels = list(ro.index.levels[0])
        self.channel_matchers = []
        for ch in self.channels:
            if ref_final is None:
                self.channel_matchers.append(FMMatcher(ro.loc[ch], ref.loc[ch], eps_xy, eps_z))
            else:
                self.channel_matchers.append(FMMatcher(ro.loc[ch], ref.loc[ch], ref_final.loc[ch], eps_xy, eps_z))


    def auto_set_params(self):
        for ch_al in self.channel_matchers:
            ch_al.auto_set_params()
    ...
    def align_refs(self):
        ...
        ref_ch1 = []
        ref_ch2 = []
        for i, ch1 in enumerate(self.channel_matchers):
            for j in range(i+1, len(self.channels)):
                ref_ch1.append(ch1)
                ref_ch2.append(self.channels[j])
                cross_ch_al = FMMatcher(self.channel_matchers[i].ref, self.channel_matchers[i].ro, self.channel_matchers[j].ref)


                (opt_matches, mm_lt, mm_zt, x_offset, y_offset, z_offset, lat_offset) = cross_ch_al.auto_set_params()
                ref_matches.append(opt_matches)
        self.ref_matches_df = pd.DataFrame({"ch1":ref_ch1, "ch2":ref_ch2, "matches":opt_matches})
        return self.ref_matches_df
...
class FMMatcher:
    ...
    :param ref_final: Optional- pandas dataframe of final reference points measured after readout hybridizations. Used
    ...
    def __init__(self, ro, ref, ref_final=None, eps_xy=0, eps_z=0):
        """
        Initialize RefMatcher object. Save pandas DataFrames of reference and readout points
        ...
        self.ro = ro
        self.ref = self.remove_double_reference_markers(ref, eps_xy, eps_z)
        ...
        if type(ref_final) == pd.DataFrame:
            self.ref_final = self.remove_double_reference_markers(ref_final, eps_xy, eps_z)
    ...
    def auto_set_params(self):
        ...
        :return: tuple(opt_matches :: pd.DataFrame, mm_lt, mm_zt, x_offset, y_offset, z_offset, lat_offset)
        """
        ...
        for i, lat_error in enumerate(lat_errors):
            for j, z_error in enumerate(z_errors):
                ...
                ral = FMMatcher(ref_hyb_indexed, deepcopy(self.ref_final))
    ...
    def load_saved_parameters(self, params_file_name):
        """
        Load previously found and saved parameters to the FMMatcher object.

        :param params_file_name:
        :return: None
        """
        self._set_saved_parameters(pd.read_csv(params_file_name))

    def _set_saved_parameters(self, params_df):
    ...
    def set_n_longest_edges(self, nle):
        """
        Sets the number of longest edges to search through to find initial pair of fiducial markers.

        :param nle: The number of pairs of longest edges to search through to find initial pair of fiducial markers.
        :return: None
        """
    ...
    def set_xy_search_error(self, xyse):
        """
        Sets the allowed lateral error when matching pairs of fiducial markers.
    ...
    def set_z_search_error(self, zse):
        """
        Sets the allowed z error when matching pairs of fiducial markers.
    ...
    def set_min_edge_match(self, min_edge_matches):
        """
        Set the minimum number of pairs of fiducial markers that must be matched to consider the fiducial markers
        pattern matched.

        :param min_edge_matches:
        :return: None
        """
    ...
    def set_min_bright_prop(self, mbp):
        """
        The set the minimum proportion of brightness between fiducial markers in the readout hybridations to in the reference
        image.

        :param mbp: the minimum proportion of brightness between fiducial markers in the readout hybridations to in the reference
        image.
        :return: None
        """
    ...
    def set_max_bright_prop(self, mbp):
        """
        Sets tne maximum proportion of brightness between fiducial markers in the readout hybridiztions to the reference image.
    ...
    def set_n_unmatch_give_up(self, nugu):
        """
        If after finding an initial pair of fiducial markers, the algorithm searches for this many other fiducial markers
        without finding any, assume that the initial match was in error, and proceed to search for a new initial match.
    ...
    def set_min_dot_matches(self,mdm):
        """
        At least this many dots must be matched to consider a fiducial marker pattern in a readout image matched to the reference
        pattern.

        :param mdm: The minimum number of matched dots in a matched hybridization image fiducial marker pattern
        :return: None
        """
    ...
    def set_max_lat_offset(self, mlo):
        """
        Sets an upper bound on the lateral drift from the reference image to each readout hybridization to help narrow
        the search for initial pairs of fiducial markers (optional).

        :param mlo: the maximum allowed lateral (xy) offset from reference
        :return: None
        """
    ...
    def set_max_z_offset(self, mzo):
        """
        Sets an upper bound on the z drift from the reference image to each readout hybridization to help narrow
        the search for initial pairs of fiducial markers (optional).

        :param mzo: the maximum allowed z offset from reference
        :return: None
        """
    ...
    def set_outlier_sd_thresh(self, ost):
        """
        Matched fiducial markers that have drifted more than this many standard deviations difference that the mean distance
        that all matched have drifted will be removed.

        :param ost: the z-score threshold for outliers
        :return:
        """
    ...
    def set_min_fm_hyb_matches(self, mfhm):
        """
        Fiducial markers that were matched in fewer hybridizations than this will be discarded.

        :param mfhm: the minimum fidicuial marker hyb marches
        :return:
        """
    ...
    def align(self, hybs=None):
        ...
        #self._find_pairwise_affine_tforms()
    ...
    def save_offsets(self, filename):
        """
        Saves alignment offsets found to csv file.

        :param filename: name of the csv file to which to save.
        :return:
        """
        self.offsets.to_csv(filename, index=True)

    def save_pairwise_tforms(self, filename):
        """
        Saves a for dimensional array of affine transforms between hybridizations to a npy file.
        The first two dimensions are for each of the pairwise hybridizations. The Matrix returned
        in the index [i, j, :, :] gives the output of the [estimateAffine3D](https://docs.opencv.org/3.4/d9/d0c/group__calib3d.html#ga396afb6411b30770e56ab69548724715)
        function of OpenCV when estimating an affine transformation between hybridization i and hybridization j.

        :param filename: name of the csv file to which to save.
        :return:
        """
        np.save(filename, self.pairwise_affine_tforms)

    def save_loocv_errors(self, filename):
        """
        Saves leave-one-out cross validation error estimates to a csv file.

        :param filename: Name of the file to which to save error estimates.
        :return:
        """
    ...
    def save_matches(self, save_name):
        """
        Save csv file of fiducial marker matches between the reference and each readout hybridization.

        :param save_name: Name of the file to which to save matches.
        :return:
        """
    ...
    def save_params(self, save_name):
        """
        Saves the parameters from the FMMatcher object to a csv file.

        :param save_name: The file name which to save the parameters to.
        :return:
        """
        params = pd.DataFrame()
    ...
    def remove_double_reference_markers(self, ref, xy_r, z_r):
        """
        Removes fiducial markers from the reference list that are within given lateral and z radii of each other.

        :param ref: dataframe of fiducial markers in the reference image
        :param xy_r: minimum allowed lateral separation of fiducial markers
        :param z_r: minimum allowed z separation of fiducial markers
        :return:
        """
        ref.reset_index(inplace=True)
        tree = KDTree(ref.loc[:, 'x':'y'])
        pairs = tree.query_pairs(xy_r)
        pairs = [list(pair) for pair in pairs if abs(ref.loc[pair[0],'z']-ref.loc[pair[1],'z']) <= z_r]
        to_remove = np.unique(sum(pairs, []))
        return ref.drop(to_remove)
    ...
    def _est_offsets_all_matches(self, _matches):
        ...
        matches.loc[:, "x_offset"] = matches['comp_x'] - matches['ref_x']
        matches.loc[:, "y_offset"] = matches['comp_y'] - matches['ref_y']
        matches.loc[:, "z_offset"] = matches['comp_z'] - matches['ref_z']
        mfg = matches.groupby('hyb')
        means = mfg.mean()
        offsets = means[['x_offset', 'y_offset', 'z_offset']]
        return offsets

    def _find_pairwise_affine_tforms(self):
        hyb_matches = list(self.matchesDF.groupby("hyb"))
        nhybs = len(hyb_matches)
        self.pairwise_affine_tforms = np.zeros((nhybs, nhybs, 3, 4))

        for h1 in range(nhybs):
            for h2 in range(nhybs):
                h1_matches = hyb_matches[h1][1].loc[:, ["comp_x", "comp_y", "comp_z"]]
                h2_matches = hyb_matches[h2][1].loc[:, ["comp_x", "comp_y", "comp_z"]]

                merged = pd.merge(h1_matches, h2_matches, how="inner", on=["ref_x", "ref_y", "ref_z"])

                tform = cv2.estimateAffine3D(np.array(merged.loc[:, ["comp_x_x", "comp_y_x", "comp_z_x"]]),
                                             np.array(merged.loc[:, ["comp_x_y", "comp_y_y", "comp_z_y"]]))
                self.pairwise_affine_tforms[h1, h2, :, :] = tform[1]


    def est_error_loocv(self):
        """
        ...
        for fm_ref in np.unique(matches.index):
            ...
            drpd_fm = deepcopy(pd.DataFrame(matches.loc[fm_ref]))
            drpd_fm.set_index("hyb", drop=False, inplace=True)
            drpd_fm.loc[:, "loocv_align_x"] = drpd_fm["comp_x"] - loo_offsets.loc[drpd_fm["hyb"]]["x_offset"]
            drpd_fm.loc[:, "loocv_align_y"] = drpd_fm["comp_y"] - loo_offsets.loc[drpd_fm["hyb"]]["y_offset"]
            drpd_fm.loc[:, "loocv_align_z"] = drpd_fm["comp_z"] - loo_offsets.loc[drpd_fm["hyb"]]["z_offset"]
            drpd_fm.loc[:, "x_diff"] = drpd_fm["loocv_align_x"] - drpd_fm["ref_x"]
            drpd_fm.loc[:, "y_diff"] = drpd_fm["loocv_align_y"] - drpd_fm["ref_y"]
            drpd_fm.loc[:, "z_diff"] = drpd_fm["loocv_align_z"] - drpd_fm["ref_z"]

            #loocv_alignments = loocv_alignments.append(drpd_fm[["ref_x", "ref_y", "ref_z", "ref_int", "hyb",
            #                                   "loocv_align_x", "loocv_align_y", "loocv_align_z", "comp_int",
            #                                    "x_diff", "y_diff", "z_diff"]])
            loocv_alignments = pd.concat([loocv_alignments, drpd_fm[["ref_x", "ref_y", "ref_z", "ref_int", "hyb",
                                                                "loocv_align_x", "loocv_align_y", "loocv_align_z",
                                                                "comp_int", "x_diff", "y_diff", "z_diff"]]])

        self.loocv_alignment_errors = loocv_alignments
    ...
    def _find_ref_edges(self, n_longest, ref_df = None):
        ...
        ref_dists.loc[:, 'ul_dist'] = 0
        ref_dists.loc[:, 'ur_dist'] = 0
        ref_dists.loc[:, 'bl_dist'] = 0
        ref_dists.loc[:, 'br_dist'] = 0
        ...
        for i, dot in self.ref.iterrows():
            ...
            ref_dists.loc[i, 'ul_dist'] = dot.x ** 2 + dot.y ** 2
            ref_dists.loc[i, 'ur_dist'] = (2048 - dot.x) ** 2 + dot.y ** 2
            ref_dists.loc[i, 'bl_dist'] = dot.x ** 2 + (2048 - dot.y) ** 2
            ref_dists.loc[i, 'br_dist'] = (2048 - dot.x) ** 2 + (2048 - dot.y) ** 2
    ...
    def align_hyb(self, hyb, ch = None, print_hyb = True):
        """
    ...
    def _traverse_reference(self, matched_ref_edge, ro_u, ro_v):
        ...
        while self.traversal_queue:
            ...
            if matched_dot_key in self.matchDict and self.matchDict[matched_dot_key] < self.min_edge_matches:
                ...
                self.ref_dots = pd.concat([self.ref_dots, matched_dot[0]])
    ...
    def _compare_dots(self, matched_dot, matched_dot_key, ref_neighbor, reserved=False):
        ...
        if np.isnan(search_x) or np.isnan(search_y):
            return False
        ...
        if len(matches) == 1:  # we found a match!
            ...
            if not reserved:
                #print("ref_neighbor_key:")
                #print(ref_neighbor_key)
                #print(self.ref_dots.loc[ref_neighbor_key,:])
                #print(self.ref_dots.loc[ref_neighbor_key, 'n_unmatched'])
                try:
    ...
    def _process_match(self, new_match, old_match, reserved=False):
        ...
        if (new_match_key[0], old_match_key[0]) not in self.edgeMatches:
            ...
            if self.matchDict[old_match_key] >= self.min_edge_matches and old_match_key[0] in self.ref_dots.index and not reserved:
                ...
                self.reserved_ref_dots = pd.concat([self.reserved_ref_dots, self.ref_dots.loc[old_match[0]]])
            ...
            if self.matchDict[new_match_key] >= self.min_edge_matches and new_match_key[0] in self.ref_dots.index and not reserved:
                ...
                #print("concatenating...")
                #print(self.reserved_ref_dots.head())
                #print(self.ref_dots.loc[new_match_key[0]].head())
                try:
                    self.reserved_ref_dots = pd.concat([self.reserved_ref_dots, self.ref_dots.loc[new_match_key[0]]])
                except:
                    pass
                try:
    ...
    def _add_hyb_to_match_df(self, hyb_num, offsets):
        ...
        matchesDF.loc[:, 'hyb'] = [hyb_num] * len(ref_y)
        matchesDF.loc[:, 'ref_x'] = ref_x
        matchesDF.loc[:, 'ref_y'] = ref_y
        matchesDF.loc[:, 'ref_z'] = ref_z
        matchesDF.loc[:, 'ref_int'] = ref_int
        matchesDF.loc[:, 'comp_x'] = comp_x
        matchesDF.loc[:, 'comp_y'] = comp_y
        matchesDF.loc[:, 'comp_z'] = comp_z
        matchesDF.loc[:, 'comp_int'] = comp_int
        matchesDF.loc[:, 'aligned_x'] = matchesDF['comp_x'] - x_offset
        matchesDF.loc[:, 'aligned_y'] = matchesDF['comp_y'] - y_offset
        matchesDF.loc[:, 'aligned_z'] = matchesDF['comp_z'] - z_offset

        #self.matchesDF = self.matchesDF.append(matchesDF)
        self.matchesDF = pd.concat([self.matchesDF, matchesDF])
