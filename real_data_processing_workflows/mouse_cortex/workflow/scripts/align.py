...
for i in range(1,11):
    ...
    if len(hnf) > 0:
        #dal.set_n_longest_edges(dal.n_longest_edges*10)
        dal.min_dot_matches = 4
        dal.set_min_edge_match(2)
        dal.set_xy_search_error(dal.xyse + 0.5)
        #dal2.align()
        for hyb in hnf['hyb']:
            dal.offsets.loc[hyb, 'x':] = dal.align_hyb(hyb)
    else:
        break
...
dal.est_error_loocv() #dal.matchesDF)
