...
n_pseudocolors = zeros_probed ? q : q - 1

hyperstack = zeros(n*n_pseudocolors,1,fov_width,fov_width)
rounds = []
pseudocolors = []
...
psfs = fill(psf, n*n_pseudocolors)
psf_depths = ones(n*n_pseudocolors)
psf_heights = 6 .* ones(n*n_pseudocolors)
psf_widths = 6 .* ones(n*n_pseudocolors)
min_intensities = 8000.0 .* ones(n*n_pseudocolors)
