...
zeros_probed = substr(code, 1, 7) == "seqFISH"
n_pseudocolors = if (zeros_probed) q else q - 1
...
Y = array(0, dim=n*n_pseudocolors*fov_width*fov_width)
...
for (j in 1:nrow(cpaths)){
  ...
  for (dot in dot_array){
    ...
    s = n_pseudocolors*(block-1) + pseudocolor
