for code in codes_to_simulate["code"]:
    ...
    rule:
        ...
        output:
            "results/graph_iss/decoded/decoded_" + code + "_rep_{rep}_nbarcodes{nbarcodes}_nnonspec{nnonspec}_pdrop{pdrop}_rstdv{rstdv}.csv"
        params:
        ...
        # Shares a group id with score_graph_iss for this same condition so cluster execution
        # submits both as one scheduler job instead of two.
        group: f"graph_iss_{code}" + "_{rep}_{nbarcodes}_{nnonspec}_{pdrop}_{rstdv}"
