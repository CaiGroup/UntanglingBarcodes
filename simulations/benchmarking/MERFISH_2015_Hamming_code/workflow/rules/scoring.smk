...
rule score_bardensr:
    ...
    wildcard_constraints:
        ...
        pdrop=r"[\d.]+",
        rstdv=r"[\d.]+"
    # Same group id as bardensr_decode for this condition so cluster execution submits both
    # as one scheduler job instead of two.
    group: "bardensr_{code}_{rep}_{nbarcodes}_{nnonspec}_{pdrop}_{rstdv}"
...
rule score_istdeco:
    ...
    wildcard_constraints:
        ...
        pdrop=r"[\d.]+",
        rstdv=r"[\d.]+"
    # Same group id as istdeco_decode for this condition so cluster execution submits both
    # as one scheduler job instead of two.
    group: "istdeco_{code}_{rep}_{nbarcodes}_{nnonspec}_{pdrop}_{rstdv}"
    conda: "../envs/python_base.yaml"
...
rule score_graph_iss:
    input:
        decoded="results/graph_iss/decoded/decoded_{code}_rep_{rep}_nbarcodes{nbarcodes}_nnonspec{nnonspec}_pdrop{pdrop}_rstdv{rstdv}.csv",
        truth="results/truth/ground_truth_{code}_rep_{rep}_nbarcodes{nbarcodes}_nnonspec{nnonspec}_pdrop{pdrop}_rstdv{rstdv}_sim.csv",
        codebook="resources/codebooks/{code}cb.csv"
    output:
    ...
    params:
        method="graph_iss"
    # Same group id as graph_iss_decode for this condition so cluster execution submits both
    # as one scheduler job instead of two.
    group: "graph_iss_{code}_{rep}_{nbarcodes}_{nnonspec}_{pdrop}_{rstdv}"
    conda: "../envs/python_base.yaml"
...
rule score_spotsfirst:
    ...
    wildcard_constraints:
        ...
        pdrop=r"[\d.]+",
        rstdv=r"[\d.]+"
    # Same group id as spotsfirst_find_barcode_candidates/spotsfirst_choose_barcodes for this
...
rule score_compressed_sensing:
    ...
    # Same group id as cs_get_candidate_coords/cs_find_cand_cpaths_with_negctrls/cs_l0learn_fit
    # for this condition so cluster execution submits them as one scheduler job instead of
    # several.
    group: "compressed_sensing_{code}_{rep}_{nbarcodes}_{nnonspec}_{pdrop}_{rstdv}"
    conda: "../envs/python_base.yaml"
